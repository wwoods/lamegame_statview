 ...
 *
 * Updated by Walt Woods on 2012-11-14 for RequireJS.  Also modified to add
 *     update() for placing "bookmarks" without performing an action. 
 */
...
return {
    ...
    update: function(newHash) {
        //Update the hash with the newHash WITHOUT calling callback.
        if (newHash == hash) return;
        if (iframe) {
            setIframe(newHash);
        } else {
